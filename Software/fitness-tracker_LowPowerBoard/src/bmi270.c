...
static bmi270_device_t bmi270_dev;
static I2C_TransferSeq_TypeDef bmi_i2cTransfer;
...
/*! @name  Global array that stores the configuration file of BMI270 */
     ...
     static const uint8_t bmi270configfile[] = {
...
unsigned int configUploadOffset = 0;

void bmi270StartInitSequence(void)
{
  bmi270_dev.i2c_cfg.i2c_port = I2C0;
  bmi270_dev.i2c_cfg.address = BMI270_ADDR_LOW;
}

void bmi270SendSoftReset(void)
{
  uint8_t cmd = SOFT_RESET_CMD;
  startWriteBMI270DataChunk(BMI270_CMD_REG, &cmd, 1);
}

void bmi270ReadChipID(void)
{
  lastReadSize = 1;
  startReadBMI270Reg(BMI270_CHIP_ID_REG, readBuffer, lastReadSize);
}

void bmi270ReadInitStatus(void)
{
  lastReadSize = 1;
  startReadBMI270Reg(BMI270_INIT_STATUS, readBuffer, lastReadSize);
}

/***************************************************************************//**
 * @brief Read accelerometer XYZ values (non-blocking)
 ******************************************************************************/
void bmi270ReadAccel(void)
{
  lastReadSize = 6;
  startReadBMI270Reg(BMI270_ACC_X_LSB, readBuffer, lastReadSize);
}

/***************************************************************************//**
 * @brief Read gyroscope XYZ values (non-blocking)
 ******************************************************************************/
void bmi270ReadGyro(void)
{
  lastReadSize = 6;
  startReadBMI270Reg(BMI270_GYR_X_LSB, readBuffer, lastReadSize);
}

/***************************************************************************//**
 * @brief Helper to get last read buffer
 ******************************************************************************/
...
/***************************************************************************//**
 * @brief Validate BMI270 chip ID read from device
 * @return true if valid, false otherwise
 ******************************************************************************/
bool isValidBMI270ChipID(void)
{
  const uint8_t *dataPtr = NULL;
  uint8_t size = getLastBMI270ReadBuffer(&dataPtr);

  if ((size >= 1) && (dataPtr[0] == BMI270_CHIP_ID)) {
    return true;
  }
  return false;
}
...
bool isBMI270InitSuccessful(void)
{
  ...
  if ((size >= 1) && (dataPtr[0] == INIT_SUCCESS_STATUS)) {
    return true;
  }
  return false;
}
...
{
    uint8_t data = value;
    startWriteBMI270DataChunk(BMI270_INIT_ADDR_0_REG, &data, 1);
}

void setInitAddr1(uint8_t value)
{
    uint8_t data = value;
    startWriteBMI270DataChunk(BMI270_INIT_ADDR_1_REG, &data, 1);
}

// Sends the next chunk of config file to BMI270 INIT_DATA register (via I2C)
void sendNextConfigChunk(void)
{
    unsigned int remaining = BMI270_CONFIGFILE_LEN - configUploadOffset;
    unsigned int chunk_size = (remaining > BMI270_CONFIG_CHUNK_MAX) ? BMI270_CONFIG_CHUNK_MAX : remaining;
    if(chunk_size == 0)
        return;
    // Use I2C write: register, data, length
    startWriteBMI270DataChunk(BMI270_INIT_DATA_REG, &bmi270configfile[configUploadOffset], chunk_size);
    configUploadOffset += chunk_size;
}

// Returns true if more config data remains
bool hasMoreConfigData(void)
{
    return configUploadOffset < BMI270_CONFIGFILE_LEN;
}

void bmi270SetInitCtrl(uint8_t value)
{
  uint8_t data = value;
  startWriteBMI270DataChunk(BMI270_INIT_CTRL, &data, 1);
}
...
{
    uint8_t data =value;
    startWriteBMI270DataChunk(BMI270_POWER_CONF,&data,1);
...
{
  timerWaitUs_interrupt(100000); // 100ms wait
}

void waitForSoftReset(void)
{
  timerWaitUs_interrupt(2000); // Assuming 2ms wait
}
...
// Helper: chunked I2C write to INIT_DATA
void startWriteBMI270DataChunk(uint8_t reg, const uint8_t *data, unsigned int len)
{
    bmi_i2cTransfer.addr        = bmi270_dev.i2c_cfg.address << 1;
    bmi_i2cTransfer.flags       = I2C_FLAG_WRITE;

    temp[0] = reg;  // first byte is register address
    memcpy(&temp[1], data, len);
    bmi_i2cTransfer.buf[0].data = temp;
    bmi_i2cTransfer.buf[0].len = len + 1;
    bmi_i2cTransfer.buf[1].data = NULL;
    bmi_i2cTransfer.buf[1].len = 0;
...
{
    bmi_i2cTransfer.addr        = bmi270_dev.i2c_cfg.address << 1;
    bmi_i2cTransfer.flags       = I2C_FLAG_WRITE_READ;

    temp[0] = reg;
    bmi_i2cTransfer.buf[0].data = temp;
    bmi_i2cTransfer.buf[0].len = 1;
    bmi_i2cTransfer.buf[1].data = buffer;
    bmi_i2cTransfer.buf[1].len = len;
