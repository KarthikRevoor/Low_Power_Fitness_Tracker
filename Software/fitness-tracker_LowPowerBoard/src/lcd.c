...
void displayInit()
{
    ...
    //
    gpioTempSensorOn(); // we need SENSOR_ENABLE=1 which is tied to DISP_ENABLE
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  // Set up a 1Hz timer (1 second interval)
	  timer_response =sl_bt_system_set_lazy_soft_timer(32768,32768, 2, 0);
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("Soft timer() returned non-zero error code=0x%04x",(unsigned int) timer_response);
     }
